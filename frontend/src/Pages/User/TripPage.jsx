...
function TripPage() {
  ...
  return (
    <>
      <div className='min-h-screen bg-cover' style={{ backgroundImage: `url(${home1})`, backgroundSize: 'cover' }}>
        <Header />
        <div className="flex flex-col items-center justify-center flex-grow mt-32 cherry-bomb text-black text-4xl decoration-red-800">
          ...
          <p className=" items-center text-center  text-black  text-xxl">
            Your Next Destination Starts Here: Explore Our Exclusive Packages with BACKPACKERZ!
          </p>
        </div>
      </div>
              ...
              {packages.map((packageItem) => (
                <div key={packageItem.id} className="group relative">
                  <div className="relative h-80 w-full overflow-hidden rounded-lg bg-white sm:aspect-h-1 sm:aspect-w-2 lg:aspect-h-1 lg:aspect-w-1 group-hover:opacity-75 sm:h-64">
                    <Link to={`/trip-details/${packageItem.id}`} className="block relative h-80 w-full overflow-hidden rounded-lg bg-white sm:aspect-h-1 sm:aspect-w-2 lg:aspect-h-1 lg:aspect-w-1 group-hover:opacity-75 sm:h-64">
                      <img
                  ...
                  <p className="text-base font-semibold text-gray-900">{packageItem.destination}</p>
                  <p className="text-base font-semibold text-gray-900">{packageItem.price}₹</p>
                </div>
              ))}
            </div>
          </div>
